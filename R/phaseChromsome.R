...
phaseChromosome <- function(inputfolder, outputfolder='./StrandPhaseR_analysis', positions=NULL, WCregions=NULL, chromosome=NULL, pairedEndReads=TRUE, min.mapq=10, min.baseq=30, num.iterations=2, translateBases=TRUE, fillMissAllele=NULL, splitPhasedReads=FALSE, callBreaks=FALSE, exportVCF=TRUE, bsGenome=NULL) {
  ...
  if (splitPhasedReads) {
    haps.gr <- splitReads(data.object=assem.haps, inputfolder=inputfolder, pairedEndReads=pairedEndReads, min.mapq=min.mapq)
  ...
  if (callBreaks) {
    ...
    breakpoints <- runBreakpointr(input.data = phased.haps, pairedEndReads=pairedEndReads, chromosomes=chromosome, windowsize=15000, binMethod="size", pair2frgm=TRUE)
