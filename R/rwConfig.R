...
writeConfig <- function(conf, configfile) {
	...
	cat("\n[StrandPhaseR]\n", file=f)
	for (i1 in c('positions', 'WCregions', 'min.baseq', 'num.iterations', 'translateBases', 'fillMissAllele', 'splitPhasedReads', 'compareSingleCells', 'callBreaks', 'exportVCF', 'bsGenome', 'ref.fasta')) {
		cat(i1," = ",formatstring(conf[[i1]]),"\n", file=f)
	}
	close(f, type='w')
}
