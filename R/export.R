...
#' @param outputDirectory Location to store created bedfile(s)
...
#' @param outputDirectory Location to store created VCF file(s)
...
exportVCF <- function(index=NULL, outputDirectory, phasedHap=NULL, bsGenome, chromosome=chr) {
  ...
  df <- data.frame(chr, snv.pos, id, ref.alleles, alt.alleles, qual, filter, info, format, genotypes)
  
  savefile.vcf <- file.path(outputDirectory, paste0(chromosome, '_phased.vcf'))
